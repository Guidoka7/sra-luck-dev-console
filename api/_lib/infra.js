...
function buildSignals({supabase,devSupabase,cloudflare,runtime}){
 ...
 if(sm?.swap?.usagePercent!=null)add('supabase','swap_usage_percent','Supabase · Swap',sm.swap.usagePercent,'%',5,20,{raw:sm.swap});
 if(sm?.disk?.usagePercent!=null)add('supabase','disk_usage_percent','Supabase · Disco',sm.disk.usagePercent,'%',75,85,{raw:sm.disk});
 ...
 if(dm?.swap?.usagePercent!=null)add('dev_supabase','swap_usage_percent','Supabase Dev · Swap',dm.swap.usagePercent,'%',5,20,{raw:dm.swap});
 if(dm?.disk?.usagePercent!=null)add('dev_supabase','disk_usage_percent','Supabase Dev · Disco',dm.disk.usagePercent,'%',75,85,{raw:dm.disk});
