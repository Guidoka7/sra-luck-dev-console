...
- RD Station no Dev Console: credenciais OAuth ficam no cofre e nunca retornam em texto puro; a autorização usa `GET …/rd-station/authorize-url`; a ativação só fica disponível com credenciais obrigatórias, OAuth autorizado e persistência pronta, e o backend executa validação autenticada real antes de marcar como ativa.
- Monitoramento do RD usa apenas dados existentes: última importação, latência do último teste autenticado, contagens de execuções/itens/duplicidades/falhas e eventos de `/api/admin/integrations/historico`. O backend do RD não possui fila genérica de retries; o Dev Console mostra isso explicitamente em vez de inventar retentativas.
- A API CRM v2 do RD suporta operações de escrita, mas o Sra Luck mantém o RD em **somente leitura por regra do projeto**; portanto o Dev Console não expõe criar/alterar contatos ou negociações.
